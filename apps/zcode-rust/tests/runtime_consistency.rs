...
impl SessionStore for Store {
    ...
    async fn snapshot_attachment(
        ...
        _: &str,
        mime: &str,
    ) -> Result<zcode_rust::domain::session::StoredAttachment> {
        Ok(zcode_rust::domain::session::StoredAttachment {
...
impl ToolPort for Tools {
    ...
    async fn execute(
        ...
        if name == "CleanupFailure" {
            return Err(zcode_rust::contract::ProcessCleanupFailure.into());
        }
        if n == 1 {
