...
use zcode_rust::contract::{ModelIdentity, ModelPort, ModelRegistry, RuntimePorts};
use zcode_rust::{
    adapters::{
        self,
...
async fn main() {
    if let Err(error) = run().await {
        ...
        use std::io::Write;
        let _ = writeln!(std::io::stderr().lock(), "zcode-rust: {error}");
        std::process::exit(1);
    }
}
async fn run() -> Result<()> {
    let args = Args::parse();
    let question_timing = adapters::config::question_timing()?;
    ...
    let requested_data = args.data_dir.unwrap_or_else(|| {
        std::env::var_os("ZCODE_RUST_DATA_DIR")
    ...
    let workspace = std::env::var("ZCODE_WORKSPACE_IDENTITY")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .map(|v| v.trim().to_owned())
        .unwrap_or_else(|| requested_cwd.to_string_lossy().into_owned());
    ...
    if !args.prepare_storage {
        ...
        let imported = async {
            if let Some(source) = adapters::legacy_paths::resolve(
                args.import_ts_db,
                &requested_cwd,
                args.config.is_none(),
            )
    ...
    if args.prepare_storage {
        ...
        let registry = if config.is_none() {
            adapters::registry::Registry::from_env()
                .await?
        ...
        let model = config
            .map(adapters::provider::HttpModel::new)
            .map(|m| Arc::new(m) as Arc<dyn ModelPort>);
        Engine::new(
            workspace,
            identity,
            RuntimePorts {
                context: Arc::new(adapters::context_source::WorkspaceContext::new(
                    cwd.clone(),
                    std::env::var_os("HOME")
                        .filter(|s| !s.is_empty())
                        .or_else(|| std::env::var_os("USERPROFILE"))
                ...
                store: Arc::new(store),
                model,
                tools: Arc::new(adapters::tools::WorkspaceTools::new(
                    cwd,
                    data_dir.join("tool-results"),
                )),
                clock: Arc::new(adapters::SystemClock),
            },
        )
        .await?
