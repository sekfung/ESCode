//! 图片模型预算压缩（docs/specs/rust-media-read.md），按 TS Jimp 适配器 `prepareJimpImageForModel`
//! 的候选顺序与尺寸规则；编码器不同，压缩后的字节与 TS 不逐字相同。Read、Composer 图片附件与
//! 工具结果媒体共用这一实现（TS 同样共用 `ImageProcessorPort.prepareForModel`）。
