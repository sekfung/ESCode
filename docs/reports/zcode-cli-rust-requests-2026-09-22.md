...
运行命令：`pnpm test:zcode-cli-rust`、`pnpm check:zcode-cli-rust`、`pnpm typecheck`、`pnpm lint`、`pnpm architecture:check --changed`。测试脚本重建真实 debug 二进制，无个人账号或外部模型请求。
...
node scripts/bench-zcode-cli-rust-suite.mjs \
