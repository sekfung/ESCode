...
- Rust 自己的会话写在 `<dataDir>/rust-sessions.sqlite`。**回退到 Node 后这些会话在 Node 侧不可见**（Node 不读该库）；数据仍在磁盘上，重新启用 Rust 即可再次打开。2026-09-26 用户确认：该单向模型满足发布门槛，不实现 Rust→TS 反向导出；切换前的历史回退无损。
...
## 功能缺口范围（2026-09-25 用户决定）

| 缺口                                       | 决定                                                                                          |
| ------------------------------------------ | --------------------------------------------------------------------------------------------- |
| 动态工作流                                 | 暂声明 unsupported（此前决定）                                                                |
...
| 自定义斜杠命令                             | 已实现，与 Node 差分一致（rust-custom-commands.md）                                           |
| 主代理记忆索引 / 提取                      | 已实现，与 Node 差分一致（rust-project-memory.md）                                            |
| MCP OAuth、媒体 Read（大图缩放、PDF 等）   | 实现                                                                                          |

## 发布门槛（全部通过才切换默认 runtime）

| 维度     | 验收方式                                                                           | 现状                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 功能对齐 | App 集成套件 + 各能力差分语料（权限、Bash、代理、prompt 等）                       | 三平台 CI 243 用例通过、Edit 宽松匹配已与 TS 差分对齐（rust-edit-matching.md，2026-09-25）；模型可见工具面（名称/顺序/描述/embedded search 与 Bash prelude）已与 Node 请求逐字对齐（rust-tool-surface.md）；WebFetch 已实现并与 Node 实网差分一致（rust-webfetch.md）；ReadSessionContext（含跨 workspace、TS 库回落、`#sess_*` 提醒、`sess_` 会话 id）已实现并与 Node 差分一致（rust-read-session-context.md）；Cron 工具与自动化轮守卫已实现并与 Node 差分一致（rust-cron.md）；自定义斜杠命令（目录、参数与 shell 展开、插件命令、失败 ACK）已实现并与 Node 差分一致（rust-custom-commands.md）；项目记忆（Memory 段、MEMORY.md 索引、记忆写入放行与来源补写、轮次后自动提取）已实现并与 Node 差分一致（rust-project-memory.md）；output style / 自定义 system 在 App 链路无入口，无需实现；媒体 Read（图片预算压缩、PDF 原生与页渲染、视频）、工具结果媒体投影、附件大图缩放与来源路径已实现并与 Node 差分一致（rust-media-read.md 第 1–4 期）；MCP 工具结果格式与图片结果（含超预算图片 artifact）已对齐（rust-mcp-parity.md 第 1–2 期）；会话标题与 Goal 摘要标题（`titleFromInput` 公式、辅助模型 sidecar、`/goal` 即时生成与兜底摘要、`custom` 优先与首条输入回退校验）已实现并与 Node 差分一致（rust-session-title.md，2026-09-26）；WebSearch（provider-native 搜索，仅声明 `supportsNativeWebSearch` 的 Anthropic 模型）已实现并与 Node 差分一致（rust-websearch.md）；MCP OAuth 授权码 + PKCE 已实现并与 Node 差分一致（授权、跨 runtime 凭据复用、refresh、invalid_grant；rust-mcp-oauth.md，2026-09-26），client_credentials 已实现并与 Node 差分一致，官方 MCP（http 身份头、stdio `_meta`）已与 Node 差分一致；**仍缺**：浏览器截图压缩（rust-mcp-parity.md 第 4 期，依赖未提供的浏览器工具）；跨日 reminder 已实现（rust-date-change.md）；差分中新发现：官方插件 seed 已实现并与 Node 双向字节一致（rust-official-plugin-seed.md）；新会话 system prompt 的 Chat 合并与 Shell 名称已对齐（rust-request-context.md）；工作流按用户决定声明 unsupported，其余缺口的范围见下节 |
| 性能     | 首段延迟、会话加载、空闲驻留、大历史查询（见 rust-runtime-performance.md）         | 真实模型服务（自建 DeepSeek 与公网供应商）、长历史重开均已测，见 rust-perf-2026-09-25-real-provider；三平台原生 release 基准已由 CI `bench` job 实测（run 36237407731）：Linux/macOS/Windows 上 Rust 启动 8/28/114ms vs Node 990/1310/1604ms，空闲内存 12.7/11.5/8.6MiB vs 362/401/264MiB（rust-runtime-performance.md）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 跨平台   | 三平台各自构建并跑同一套件                                                         | **Windows（MSVC）/ macOS（arm64）/ Linux CI 全部通过**；6 目标 release 与 4 个安装包构建通过（rust-ci.md）；macOS/Linux 安装包未真机运行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 发布回退 | 本文两条自动化用例 + 回退步骤演练                                                  | 用例通过；安装包内置 Rust 与 Node 两种 runtime 已在产物中核实；回退后 Rust 期会话在 Node 侧不可见，已由用户确认可接受（不做反向导出）；未做真实发布演练                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
