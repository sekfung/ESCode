...
| 维度     | 验收方式                                                                           | 现状                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| -------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 功能对齐 | App 集成套件 + 各能力差分语料（权限、Bash、代理、prompt 等）                       | 三平台 CI 243 用例通过、Edit 宽松匹配已与 TS 差分对齐（rust-edit-matching.md，2026-09-25）；模型可见工具面（名称/顺序/描述/embedded search 与 Bash prelude）已与 Node 请求逐字对齐（rust-tool-surface.md）；WebFetch 已实现并与 Node 实网差分一致（rust-webfetch.md）；**仍缺**：ReadSessionContext、Cron 工具（Node 默认暴露）、WebSearch（仅模型支持原生搜索时）、output style、大图缩放/媒体 Read、MCP OAuth、自定义斜杠命令、主代理记忆索引/提取；工作流/Cron/自动任务按用户决定声明 unsupported |
