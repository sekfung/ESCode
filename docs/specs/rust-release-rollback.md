...
| 性能     | 首段延迟、会话加载、空闲驻留、大历史查询（见 rust-runtime-performance.md）         | Linux 协议开销/内存（rust-perf-2026-09-24）；**真实模型服务（自建 DeepSeek，release，short/tool/long 各 20 轮交替）：就绪 2031→101ms、冷首轮首个输出 3670→303ms、热轮 534→281ms（p50），p95/p99 与长回复见 rust-perf-2026-09-25-real-provider**；基准中发现并修复推理字段缺陷；公网供应商、长历史、MSVC/三平台 release 未测 |
