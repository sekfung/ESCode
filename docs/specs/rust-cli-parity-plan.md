...
最终交付是能够替换 App 当前 Agent 进程的 Rust runtime，入口为 `app-server --stdio`，并让 Rust TUI 复用同一个 SessionRuntime。App 保留现有输入、模型设置、权限交互、历史、工具显示及远程连接方式。
...
剪枝：非 yolo 权限模式按用户范围排除；TUI 首期只要求创建/恢复、发送文本、流式事件、stop、queue、错误和 snapshot 恢复，未启用的可选工具 profile 不要求暴露，但不能据此跳过 App 实际启用的工具。上述新增验收是计划，不把既有 23 个 Rust / 57 个 App 测试计作新能力已通过。
