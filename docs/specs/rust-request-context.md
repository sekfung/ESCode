...
- 工作目录（2026-09-25 修订）：提示词 `Primary working directory`、AGENTS 与 Git 查找使用 Host 传入的 `--cwd`
  原样（转绝对路径），与 Node 一致；此前 Rust 用 realpath，macOS `/var`→`/private/var`、Windows 8.3 短名会与 Node 不同。
