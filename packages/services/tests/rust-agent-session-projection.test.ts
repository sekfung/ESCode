...
test("Rust local and remote snapshots preserve workspace identity through cold reads", async () => {
  for (const identity of [undefined, "ssh:fixture:/project"]) {
    ...
    try {
      const h = f.start(identity), id = await h.create();
      await h.subscribe(`conversation/${id}`);
      await h.command(h.envelope("sendText",id,{text:"identity"}));
      await h.completed(id);
      const read = async (owner: typeof h) => {
        const snapshot = await owner.client.request("session/read",{sessionId:id,deliveryKind:"desktop-continuous"},zcodeSessionStateSnapshotSchema);
...
test("Task index repairs legacy local path identity in read projections and preserves remote isolation and shell state", async () => {
  ...
  try {
    const local = {taskId:"same",traceId:"fixture",workspacePath:f.cwd,workspaceIdentity:f.cwd,title:"kept",mode:"yolo" as const,provider:"glm" as const,createdAt:1,updatedAt:2};
    ...
    const structure = await repo.queryGroupedTaskViewStructure({workspaceScopes:[local,remote]});
    assert.equal(structure.members.find(m => m.workspaceKey === f.cwd)?.workspaceIdentity,undefined);
    assert.equal(structure.members.find(m => m.workspaceKey === remote.workspaceIdentity)?.workspaceIdentity,remote.workspaceIdentity);
    await repo.updateTaskState({...local,patch:{pinned:true,archived:true,title:"user title",titleOverridden:true}});
    ...
    const stored = db.prepare("SELECT pinned,archived,title,title_overridden FROM tasks WHERE workspace_key=?").get(f.cwd);
    assert.deepEqual({...stored},{pinned:1,archived:1,title:"user title",title_overridden:1});
    db.close();
  } finally { repo.close(); await f.close(); }
});
