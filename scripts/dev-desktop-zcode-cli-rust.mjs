...
if (values.help) {
  console.info(
    "Usage: pnpm dev:desktop:zcode-cli-rust [--config <fixture model.json>] [--data-dir <isolated experiment directory>] [--debug] (default: release)",
  );
...
await run(process.execPath, [resolve(root, "scripts/dev-desktop-env.mjs"), "production"], {
  ...
  ZCODE_AGENT_SERVER_COMMAND: resolve(
    root,
    `apps/zcode-cli-rust/target/${profile}/zcode-cli-rust${process.platform === "win32" ? ".exe" : ""}`,
  ),
