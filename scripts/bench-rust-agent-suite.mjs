...
if (!baseline || !candidate)
  throw new Error(
    "Usage: node scripts/bench-rust-agent-suite.mjs <baseline> <candidate> [output-directory] [candidate-window] [baseline-window] [candidate-api] [baseline-api]",
  );
    ...
    for (const [version, binary] of versions) {
      ...
      const sample = await new Promise((done, fail) => {
        const child = spawn(
          process.execPath,
          [
            resolve("scripts/bench-rust-agent.mjs"),
            binary,
